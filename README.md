...
| FlaneurWorkingDirectory | The working directory to which all commands and files are relative. | A directory path. |
...
| FlaneurBundlePrintUrlCommand | A command that prints the URL for the wapp to be bundled. | Command line expression that prints a URL. |
| FlaneurHttpRunCommand | A command that serves a wapp over HTTP. | Command line expression |
| FlaneurHttpPrintUrlCommand | A command that prints the URL for the wapp to be served over HTTP. | Command line expression that prints a URL. |
