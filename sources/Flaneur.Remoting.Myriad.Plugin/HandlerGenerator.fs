...
module Attributes =
  ...
  type RemotableAttribute() = inherit System.Attribute()
...
module private Ast =
  let isInterface =
    function
    | SynTypeDefn (_, SynTypeDefnRepr.ObjectModel (SynTypeDefnKind.Interface, _, _),_,_,_,_) -> true
    | SynTypeDefn (_, SynTypeDefnRepr.ObjectModel (SynTypeDefnKind.Unspecified, members, _),_,_,_,_)
      when members |> List.forall (function SynMemberDefn.AbstractSlot (_, _, _) -> true | _ -> false) -> true
    | _ -> false

  let isClass =
    function
    | SynTypeDefn (_, SynTypeDefnRepr.ObjectModel (SynTypeDefnKind.Class, _, _),_,_,_,_) -> true
    | SynTypeDefn (_, SynTypeDefnRepr.ObjectModel (SynTypeDefnKind.Unspecified, members, _),_,_,_,_)
      when members |> List.exists (function SynMemberDefn.ImplicitCtor (_, _, _,_,_,_) -> true | _ -> false) -> true
    | _ -> false


module private Runtime =
  let [<Literal>] Namespace = "Flaneur.Remoting"
...
module private Extract = 
  ...
  let chooseInterface =
    function
    | SynTypeDefn (SynComponentInfo.SynComponentInfo(_,_,_,ident,_,_,_,_), SynTypeDefnRepr.ObjectModel (SynTypeDefnKind.Interface, members, _),_,_,_,_)
    | SynTypeDefn (SynComponentInfo.SynComponentInfo(_,_,_,ident,_,_,_,_), SynTypeDefnRepr.ObjectModel (SynTypeDefnKind.Unspecified, members, _),_,_,_,_)
      when members |> List.forall (function SynMemberDefn.AbstractSlot (_, _, _) -> true | _ -> false) ->
      Some (ident, members)
    | _ ->
      None
  ...
  let chooseAbstractMember =
    function
    | SynMemberDefn.AbstractSlot (SynValSig(_,SynIdent(ident,_), _, synType, _,_,_,_,_,_,_,_), _, _) ->
      Some (ident, synType)
    | _ ->
      None
  ...
  let serviceSignatures ast =
    ...
    |> List.map (fun (_, typeDefs) -> 
      typeDefs |> List.filter (fun t -> Ast.hasAttribute<Attributes.RemotableAttribute> t))
